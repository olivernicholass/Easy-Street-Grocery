...
try(Connection con = DriverManager.getConnection(url, uid, pw);){
	...
	if(rst.next()){
		...
		String sql3 = "SELECT op.productId, op.quantity, w.warehouseId FROM orderproduct op JOIN productinventory pr ON pr.productId = op.productId JOIN warehouse w ON w.warehouseId = pr.warehouseId WHERE op.orderId = ?";
		PreparedStatement prepstmt4 = con.prepareStatement(sql4);
		prepstmt4.setString(1, orderId);
		ResultSet rst4 = prepstmt4.executeQuery();
		while(rst4.next()){
			String pid = rst4.getString("productId");
			String orderqty = rst4.getString("orderqty");
			String inventoryqty = rst4.getString("inventoryqty");
			...
			if(inventoryqty > orderqty){
				...
				prepstmt5.setString(1, orderqty);
		...
		String sql3 = "INSERT INTO shipment (shipmentId, shipmentDate, shipmentDesc, warehouseId) VALUES (?,?,?,?)";
		PreparedStatement prepstmt3 = con.prepareStatement(sql3); 
		prepstmt3.setString(1, orderid);
		Date currentDate = new Date();
		prepstmt3.setString(2, currentDate);
		prepstmt3.setstring(4, )
